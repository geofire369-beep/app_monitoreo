...
class MonitoraNewMonitoringPage extends StatefulWidget {
  ...
  @override
  State<MonitoraNewMonitoringPage> createState() => _MonitoraNewMonitoringPageState();
}

class _MonitoraNewMonitoringPageState extends State<MonitoraNewMonitoringPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Nuevo monitoreo'),
        ...
        builder: (context, snap) {
          ...
          return LayoutBuilder(
            builder: (context, c) {
              ...
              if (_selectedMap == null || _selectedCapilla == null) {
                final rightPanel = _RightPanel(
                  accent: accent,
                  selectedMap: _selectedMap,
                  selectedCapilla: _selectedCapilla,
                  linesWidget: _emptyLinesHint(),
                  expandBody: isWide,
                );
              ...
              final capRef = FirebaseFirestore.instance
                  .collection('monitoreo_weeks')
                  .doc(weekKey)
                  .collection('greenhouses')
                  .doc(_selectedMap!.id)
                  .collection('capillas')
                  .doc(_selectedCapilla!.id);

              return StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
                stream: capRef.snapshots(),
                builder: (context, capSnap) {
                  // 🔥 IMPORTANTE:
                  // doneKeys = remoto(Firestore) + local(Outbox)
                  return ValueListenableBuilder(
                    valueListenable: OfflineSyncService.instance.outboxListenable,
                    builder: (context, _, __) {
                      final doneKeys = <String>{};

                      // Remoto
                      if (capSnap.hasData && capSnap.data!.exists) {
                        final data = capSnap.data!.data() ?? {};
                        ...
                        if (linesAny is Map) {
                          ...
                          lines.forEach((k, v) {
                            if (v is Map) {
                              ...
                              if (status == 'FINISHED') doneKeys.add(k);
                            }
                          });
                        }
                      }

                      // Local pending (aunque no haya subido todavía)
                      final localPending = OfflineSyncService.instance.pendingLineKeys(
                        weekKey: weekKey,
                      ...
                      // Si seleccionaron una línea y se marcó done, la quitamos
                      ...
                      final bottom = (_selectedLine == null || selectedLineDone)
                          ? null
                          : SafeArea(
                              child: Padding(
                      ...
                      final linesWidgetMobile = _linesTwoSidesList(
                        doneKeys: doneKeys,
                        shrinkWrap: true,
                        scrollable: false,
                      );

                      final linesWidgetWide = _linesTwoSidesList(
                        doneKeys: doneKeys,
                        shrinkWrap: false,
                        scrollable: true,
                      );
        ...
        child: Row(
          children: [
            Container(
              ...
              height: 46,
              decoration: BoxDecoration(
                color: AppTheme.pepperGreen.withValues(alpha: 0.12),
                borderRadius: BorderRadius.circular(16),
                border: Border.all(color: AppTheme.pepperGreen.withValues(alpha: 0.18)),
              ),
              child: Icon(Icons.hub_outlined, color: AppTheme.pepperGreen),
            ),
            const SizedBox(width: 12),
            Expanded(
              child: Text(
                'Selecciona un invernadero y una capilla para ver las líneas disponibles.',
                style: TextStyle(
  ...
  Widget _linesTwoSidesList({
    ...
    if (all.isEmpty) return _emptyLinesHint();
  ...
  void _onTapLine(CapLine line, Set<String> doneKeys) {
    ...
    if (doneKeys.contains(k)) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Línea ya monitoreada (o pendiente de subir).')),
      );
      return;
    }
      ...
      child: InkWell(
        ...
        child: AnimatedContainer(
          ...
          child: Row(
            ...
            children: [
              Icon(
                done ? Icons.check_circle_rounded : Icons.view_week_outlined,
                size: 16,
                color: done
  ...
  void _startMonitoring() {
    ...
    if (found == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('No se encontró la línea seleccionada en la capilla.')),
      );
      return;
    }
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => MonitoraLineMonitoringPage(
          map: map,
          capilla: cap,
          line: found!,
        ),
            ...
            children: [
              ...
              const Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Nuevo monitoreo', style: TextStyle(fontWeight: FontWeight.w900, fontSize: 16)),
                    SizedBox(height: 4),
                    Text('Selecciona invernadero, capilla y línea para continuar.'),
                  ],
                ),
              ),
            ],
        ...
        Container(
          ...
          child: Wrap(
            ...
            children: [
              ...
              _chip(icon: Icons.view_week_outlined, label: 'Línea', value: lineLabel),
            ],
          ),
        ),
        const SizedBox(height: 12),
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Las líneas se muestran a la derecha (Sur a la izquierda, Norte a la derecha).',
                      style: TextStyle(
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('3) Líneas disponibles', style: TextStyle(fontWeight: FontWeight.w900, color: accent)),
